...
- S0 Phase B (done 2026-05-17): OLED render. `lib/oled_view/` wraps `U8G2_SSD1306_128X64_NONAME_F_HW_I2C`, two-line layout. Caller owns `Wire.begin(SDA, SCL)`. Default address 0x3C, override via `oled_view::begin(0x3D)` if needed.
- S1 Day 7b font upgrade (done 2026-05-21): swapped `u8g2_font_6x10_tf` (ASCII) for **dual unifont**: `u8g2_font_unifont_t_japanese2` (Hiragana/Katakana/Kanji) and `u8g2_font_unifont_t_vietnamese1` (Latin Extended Additional precomposed diacritics). Both are 16 px tall, so layout moved from y=0/16 to **y=0/24**. `draw_two_lines` calls `drawUTF8` (not `drawStr`) and `pick_font` switches per line by sniffing UTF-8 leading byte (0xE3-0xE9 → JP, otherwise VN). Flash cost ~+165 KB; ESP32-S3 16 MB has plenty.
...
- S0 Phase D (done 2026-05-17): `main.cpp` `onSubtitleWrite` calls `ble_protocol::decode_fragment`. Status codes (`AckStatus::Ok/Unsupported/DecodeError` = 0x01/0x02/0x03) are defined at top of `main.cpp` and mirror `.claude/skills/ble-protocol/SKILL.md`.
- S0 Phase E (done 2026-05-18): `lib/subtitle_assembler/` reassembles multi-fragment subtitles keyed by `sequence_id`. In-order only (BLE write-without-response is ordered per characteristic). Caps: `ASSEMBLED_MAX=1024` bytes, `MAX_FRAGMENTS=64`. Newer `sequence_id` silently drops in-progress buffer. `main.cpp` routes every Subtitle fragment through `g_assembler.feed`: `Incomplete` -> no ACK, wait; `Complete` -> render OLED + ACK status=0x01; `OutOfOrder/Inconsistent/Overflow` -> ACK status=0x03. Single-fragment still completes on first feed (count=1). OLED font remains ASCII only - Japanese reassembly is verified via Serial log + ACK; on-display rendering of CJK is S1+.
- S0 Phase F (in progress): ACK payload extended from 2 to 10 bytes. Layout: `status(1) | reserved(1) | t_recv_ms(4 LE) | t_render_ms(4 LE)`. `t_recv_ms` is latched on the first fragment of a new `sequence_id` (via `g_seq_in_progress` / `g_seq_recv_ms` in `main.cpp`). `t_render_ms` is `millis()` immediately after `oled_view::show_status` on Complete; `0` for Unsupported / Stale / Inconsistent / OutOfOrder / Overflow / DecodeError where no render happens. ESP32 `millis()` and the phone clock are independent - only the `t_render - t_recv` delta is meaningful as firmware processing time; RTT is measured purely on the phone side. Sequence tracking is reset on BLE disconnect.
